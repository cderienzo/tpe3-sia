...
def two(GA, population, fitness):
    ...
    parents = GA.select_mating_pool(population, fitness, 'A', Config.num_parents_mating)
    ...
    offspring.extend(GA.select_mating_pool(population_backup, fitness, 'B', len(population_backup)-Config.num_parents_mating))
    return offspring    
...
def three(GA, population, fitness):
    ...
    new_pop = GA.select_mating_pool(population_backup, fitness, 'B', len(population_backup)-Config.num_parents_mating)
    population_backup.extend(offspring)
    new_pop.extend(GA.select_mating_pool(population_backup, fitness, 'B', Config.num_parents_mating))
    return new_pop